...
const W450Details = ({ onClose }) => {
  ...
  const [product, setProduct] = useState({
    id: 5,
    name: "W450 Cashew Nuts", // 👈 must match DB name exactly
    image: W450,
    category: "Whole White",
    description: "The 'Petite Perfection' Cashew - 450 nuts per pound",
    sizes: [],
    stock: 80,
    rating: 4.4,
  });
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        console.error("Failed to fetch W450 product:", err);
      }
    };
    fetchProduct();
  }, []);
  ...
  features: (
    ...
    >
      {[
        {
          icon: "⭐",
          text: "Grade: W450 (450 nuts per pound)",
        },
        {
          icon: "💰",
          text: "Budget-friendly premium quality",
        },
        {
          icon: "👌",
          text: "Texture: Light, crisp & slightly firm",
        },
        {
          icon: "👅",
          text: "Flavor: Mild, slightly sweet & nutty",
        },
        {
        ...
        {
          icon: "⏳",
          text: "Shelf Life: 6-9 months with proper storage",
        },
      ].map((item, index) => (
        <motion.div
          key={index}
      ...
      {[
        {
          ...
          text: "Heart health: Supports good cholesterol",
        },
        {
          icon: <FaFire className="text-orange-500 text-xl" />,
          text: "Energy: Provides quick and lasting energy",
        },
        {
          icon: <FaSeedling className="text-green-500 text-xl" />,
          text: "Nutrition: Rich in magnesium & iron",
        },
        {
          icon: <FaLeaf className="text-[#2E8B57] text-xl" />,
          text: "Weight control: Keeps you full longer",
        },
        {
          icon: <FaBrain className="text-blue-500 text-xl" />,
          text: "Brain health: Improves focus & memory",
        },
        {
          icon: <FaShieldAlt className="text-purple-500 text-xl" />,
          text: "Immunity: Boosts body defense system",
        },
      ].map((item, index) => (
        <motion.div
          key={index}
  ...
  usage: (
    ...
    >
      {[
        ...
        "🍬 Used in sweets like laddu & halwa",
        "🧁 Great for baking (cookies & cakes)",
        "🥗 Add to salads & rice dishes",
        "🥜 Ideal for cashew powder & paste",
        "🎁 Best for bulk usage & gifting",
      ].map((item, index) => (
        <motion.div
          key={index}
          whileHover={{ x: 5 }}
  ...
  return (
    ...
    >
      <div
        ...
        className="hidden fixed top-4 right-4 bg-[#2E8B57] text-white px-4 py-2 text-sm sm:px-6 sm:py-3 sm:text-base rounded-lg shadow-lg z-[11000]"
      >
       ...
       <div className="px-4 sm:px-8 pt-10 sm:pt-12 pb-32 overflow-y-auto flex-1 hide-scrollbar">
          <div className="mb-6">
            <h1 className="text-2xl sm:text-3xl font-bold text-[#2E8B57] mb-2">
              W450 Cashew Nuts - Petite Perfection
            </h1>
            <p className="text-gray-600 text-sm sm:text-base">
              Budget-friendly premium quality with excellent taste
            </p>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4 sm:gap-6">
            ...
            <div className="lg:w-3/5">
              <div className="mb-6 bg-[#FAF9F6] p-4 rounded-lg">
                <h3 className="text-xl font-semibold mb-3 text-[#2E8B57]">Description:</h3>
                <p className="text-gray-700">
                  W450 cashews are whole, white kernels with a count of about
                  450 pieces per pound, making them smaller than W320 and W240,
                  but still offering premium taste and crunch. Known for their
                  delicate size, they are budget-friendly yet maintain excellent quality.
                </p>
              </div>
      ...
      {isImageZoomed && (
        ...
        >
          ...
          <div className="max-w-4xl w-full h-full flex items-center justify-center">
            <img
              src={W450}
              alt="Zoomed Whole White W450 Cashews"
