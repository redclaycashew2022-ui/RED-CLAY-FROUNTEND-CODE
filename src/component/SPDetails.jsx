...
import SPImage from "../wwimages/SP.png";
...
const SPDetails = ({ onClose }) => {
  const [isImageZoomed, setIsImageZoomed] = useState(false);
 const [product, setProduct] = useState({
    id: 6,
    name: "SP Cashews", // 👈 must match DB name exactly
    image: SPImage,
    category: "Small Pieces",
    description: "Small Pieces, Big Possibilities",
    sizes: [],
    stock: 120,
    rating: 4.2,
  });
  ...
  const selectedSize =
    ...
    const fetchProduct = async () => {
      try {
        ...
        console.error("Failed to fetch SP product:", err);
      }
    };
    fetchProduct();
  }, []);
...
const handleBuyNow = () => {
  ...
  const tabContent = {
    features: (
      ...
      >
        {[
          { icon: "🧩", text: "Type: Small Pieces from premium cashews" },
          {
            icon: "👅",
            text: "Flavor: Rich, buttery taste like whole cashews",
          },
          { icon: "💰", text: "Value: More economical than whole pieces" },
          {
            icon: "🌱",
            text: "Quality: Same premium quality as whole kernels",
          },
          {
            icon: "🌿",
        ...
        {[
          {
            ...
            text: "High in plant-based protein for energy and muscle repair",
          },
          {
            icon: <FaHeart className="text-red-500 text-xl" />,
            text: "Contains heart-healthy monounsaturated fats",
          },
          {
          ...
          },
          {
            ...
            text: "Plant-based nutrition with all essential amino acids",
          },
        ].map((item, index) => (
          <motion.div
            key={index}
    ...
    usage: (
      ...
      >
        {[
          "🍛 Ideal for curries, gravies, and thickening sauces",
          "🍪 Great for baking cookies, cakes, and nutty bread",
          "🍬 Common in sweet recipes like kaju katli, halwa, and payasam",
          "🥗 Works well as a crunchy topping for salads and desserts",
          "🥜 Used in snack mixes and granola for an extra nutty bite",
        ].map((item, index) => (
          <motion.div
            key={index}
            whileHover={{ x: 5 }}
        ...
        <div className="px-4 sm:px-8 pt-10 sm:pt-12 pb-32 overflow-y-auto flex-1 hide-scrollbar">
          ...
          <div className="mb-6">
            <h1 className="text-2xl sm:text-3xl font-bold text-[#2E8B57] mb-2">
              SP Cashews – Small Pieces, Big Possibilities
            </h1>
            <p className="text-gray-600 text-sm sm:text-base">
              All the flavor of whole cashews at a more economical price
            </p>
          </div>
            ...
            <div className="lg:w-3/5">
              ...
              <div className="mb-6 bg-[#FAF9F6] p-4 rounded-lg">
                ...
                <p className="text-gray-700">
                  SP (Small Pieces) cashews are medium-sized broken kernels
                  obtained during processing. They retain the same rich, buttery
                  taste as whole cashews, making them perfect for recipes where
                  the visual appeal of whole nuts is not essential but flavor
                  is.
                </p>
              </div>
